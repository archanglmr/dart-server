...
module.exports = class GameManager {
  constructor({gamesPath, models, socket, pauseLength = 5000}) {
    ...
    this.topLevelMenu = new GameMenuList({children: [
      this.menuItemTitle
    ]});
    ...
    for (let i = 0, c = directories.length; i < c; i += 1) {
      ...
      if (gameName) {
        ...
        this.topLevelMenu.addChild(new GameMenuItem({
          label: (this.games[gameName].info && this.games[gameName].info.name) ?
              this.games[gameName].info.name :
              gameName
        }))
      }
    }
...
//if (!state.finished)
  ...
  processAction({action, value, res}) {
    switch(action) {
      ...
      case actions.GAME_ACTION_END_GAME:
        this.menu = this.prepareTopLevelMenu();
        this.menu.show();
        this.emitGameMenu();
        this.respondSuccessful(res);
        break;
  ...
  prepareTopLevelMenu() {
    this.menuItemTitle.label = 'Game Selection';
