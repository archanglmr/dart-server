...
class Test {
  ...
  /**
   * List all the valid throws for the dart board. This is useful for building a
   * list of valid scores that can be used to help calculate the windicator.
   *
   * @returns {Array}
   */
  static listAllThrows() {
    if (!this.data) {
      this.data = [];

      // miss
      this.data.push({type: ThrowTypes.MISS, number: 0});

      // bull
      this.data.push({type: ThrowTypes.DOUBLE, number: 21});
      this.data.push({type: ThrowTypes.SINGLE_OUTER, number: 21});

      // 1-20
      for (let i = 1, c = 20; i <= c; i += 1) {
        this.data.push({type: ThrowTypes.TRIPLE, number: i});
        this.data.push({type: ThrowTypes.DOUBLE, number: i});
        this.data.push({type: ThrowTypes.SINGLE_INNER, number: i});
        this.data.push({type: ThrowTypes.SINGLE_OUTER, number: i});
      }
    }
    return this.data;
  }

  static widgetThrows(state) {
    var throws = [],
