...
module.exports = class DartGameServer {
  ...
  /**
   * Returns the the list of modifiers in a formated string good for the display
   * name. This is probably temporary.
   *
   * @param modifiers
   * @returns {string}
   */
  formatModifiers(modifiers) {
    ...
    if (Array.isArray(modifiers)) {
      for (let i = 0, c = modifiers.length; i < c; i += 1) {
        formatted.push(`[${modifiers[i]}]`);
      }
    }
    ...
    if (filter) {
      switch (filter) {
        ...
        case FilterTypes.SINGLE_INNER:
          return throwData.type === ThrowTypes.SINGLE_INNER;

        case FilterTypes.SINGLE_OUTER:
          return throwData.type === ThrowTypes.SINGLE_OUTER;
      }
    }
  ...
  isUnthrowable() {
    return (this.history.length && !this.store.getState().locked);
  }

  undoLastThrow() {
    if (this.isUnthrowable()) {
