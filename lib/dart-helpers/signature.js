...
class Signature {
  ...
  static encode(type, variation, version, modifiers) {
    ...
    return [type, variation, version, Signature.encodeModifiers(modifiers)].join('/').toLowerCase();
  }
   ...
   * @returns {{type: {string}, variation: {string}, version: {string}, modifiers: ({}|*)}|false}
   */
  static decode(signature) {
    if (signature && 'string' === typeof signature && 3 === (signature.match(/\//g) || []).length) {
      var [type, variation, version, tmpModifiers] = signature.split('/'),
          modifiers = Signature.decodeModifiers(tmpModifiers);
